...
class CUB200(Dataset):
    def __init__(self, data_set_root, image_size, transform, test_train=1, return_masks=False):
        ...
        base_dir = os.path.join(data_set_root, "CUB_200_2011")
        inner_dir = os.path.join(base_dir, "CUB_200_2011")
        if os.path.exists(inner_dir):
            base_dir = inner_dir

        class_list_path = os.path.join(base_dir, "image_class_labels.txt")
        self.data_df = pd.read_csv(class_list_path, sep=" ", names=["index", "class"])

        data_list_path = os.path.join(base_dir, "images.txt")
        cub200_df = pd.read_csv(data_list_path, sep=" ", names=["index", "file_path"])

        bbox_list_path = os.path.join(base_dir, "bounding_boxes.txt")
        bbox_df = pd.read_csv(bbox_list_path, sep=" ", names=["index", "x", "y", "width", "height"])

        # Use official train/test split 1/0
        split_path = os.path.join(base_dir, "train_test_split.txt")
        ...
        self.image_root_dir = os.path.join(base_dir, "images")
